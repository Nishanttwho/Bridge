...
void OnTimer()
{
   // LIGHTWEIGHT: Only reconnect when truly disconnected (no unnecessary checks)
   // Check if we haven't received ANY data for 60 seconds (extended timeout)
   if(isConnected && (TimeCurrent() - lastHeartbeat) > 60)
   {
      Print("[WS WARNING] No server communication for 60 seconds, checking connection...");
      
      // Simple connection test - if fails, then truly disconnected
      if(!TestConnection())
      {
         Print("[WS ERROR] Connection test failed - truly disconnected, reconnecting...");
         isConnected = false;
         if(wsHandle >= 0)
         {
            SocketClose(wsHandle);
      ...
      {
         Print("[WS] Connection test passed, updating heartbeat");
   ...
   if(isConnected && wsHandle >= 0)
   {
      ...
      // REMOVED: Account info and positions broadcasting to reduce overhead
      // Only essential trade execution logic remains
   }
}

//+------------------------------------------------------------------+
//| Test WebSocket connection without disconnecting                   |
//+------------------------------------------------------------------+
bool TestConnection()
{
   ...
   // Send a simple ping frame to test connection
   uchar pingFrame[6];
   pingFrame[0] = 0x89;  // FIN + opcode 9 (ping)
   pingFrame[1] = 0x00;  // No mask, 0 length
   ...
   if(useSSL)
   {
      sent = SocketTlsSend(wsHandle, pingFrame, 2);
   }
   else
   {
      sent = SocketSend(wsHandle, pingFrame, 2);
   }
...
void CheckForMessages()
{
   ...
   // Track when we last received data for stability check
   static datetime lastDataReceived = 0;
   lastDataReceived = TimeCurrent();
...
{
   // Check if symbol is selectable and trading is allowed
   ...
   // Check if market is open (bid/ask prices are valid)
   double bid = SymbolInfoDouble(symbol, SYMBOL_BID);
   double ask = SymbolInfoDouble(symbol, SYMBOL_ASK);
   
   if(bid <= 0 || ask <= 0)
   {
      Print("[MARKET CHECK] Invalid prices - Bid: ", bid, ", Ask: ", ask);
      return false;
   }
   ...
   double maxSpread = point * 100; // Maximum 100 points spread
   
   if(spread > maxSpread)
   {
      Print("[MARKET CHECK] Spread too wide: ", spread, " points (max: ", maxSpread, ")");
      return false;
   }
   
   // Check if we can get current prices (market is active)
   datetime lastTime = (datetime)SymbolInfoInteger(symbol, SYMBOL_TIME);
   if(TimeCurrent() - lastTime > 60) // Last price update more than 1 minute ago
   {
      Print("[MARKET CHECK] Stale prices - last update: ", TimeToString(lastTime));
      return false;
   }
   
   Print("[MARKET CHECK] Market is open - Bid: ", bid, ", Ask: ", ask, ", Spread: ", spread/point, " points");
   return true;
}
...
void ExecuteTrade(string commandId, string commandJson)
{
   ...
   {
      SendReport(commandId, false, "", "", "Market is closed for symbol: " + symbol);
      return;
   }
   ...
   // IMPROVED: Try market execution first, then fallback to pending if needed
   MqlTradeRequest request = {};
   MqlTradeResult result = {};
   
   // First attempt: Market execution (instant)
   request.action = TRADE_ACTION_DEAL;
   ...
   Print("[TRADE] Attempting market execution at: ", currentPrice);
   ...
   // Check if order was executed immediately or went to pending
   if(result.retcode == TRADE_RETCODE_DONE)
   {
      Print("[TRADE SUCCESS] Market execution: Order ", result.order, ", Deal: ", result.deal);
      SendReport(commandId, true, IntegerToString(result.order), IntegerToString(result.order), "");
   }
   else if(result.retcode == TRADE_RETCODE_PLACED || result.retcode == TRADE_RETCODE_NO_MONEY)
   {
      // Order was placed but not executed immediately - this is the pending issue
      Print("[TRADE WARNING] Order placed but not executed immediately. Retcode: ", result.retcode);
      
      // Try to modify the pending order to execute at market
      if(result.order > 0)
      {
         Print("[TRADE] Attempting to modify pending order to market execution...");
         
         MqlTradeRequest modifyRequest = {};
         MqlTradeResult modifyResult = {};
         
         modifyRequest.action = TRADE_ACTION_MODIFY;
         modifyRequest.order = result.order;
         modifyRequest.symbol = symbol;
         modifyRequest.price = currentPrice; // Set to current market price
         modifyRequest.sl = stopLoss;
         modifyRequest.tp = 0;
         modifyRequest.deviation = 0; // No deviation for modification
         modifyRequest.magic = 123456;
         modifyRequest.type_filling = brokerFillingMode;
         
         if(OrderSend(modifyRequest, modifyResult))
         {
            if(modifyResult.retcode == TRADE_RETCODE_DONE)
            {
               Print("[TRADE SUCCESS] Modified to market execution: Order ", modifyResult.order);
               SendReport(commandId, true, IntegerToString(modifyResult.order), IntegerToString(modifyResult.order), "");
            }
            else
            {
               Print("[TRADE WARNING] Modification failed, order remains pending. Retcode: ", modifyResult.retcode);
               SendReport(commandId, true, IntegerToString(result.order), IntegerToString(result.order), "Order placed but pending execution");
            }
         }
         else
         {
            Print("[TRADE WARNING] Failed to modify order, remains pending. Error: ", GetLastError());
            SendReport(commandId, true, IntegerToString(result.order), IntegerToString(result.order), "Order placed but pending execution");
         }
      }
      else
      {
         string error = "Trade placed but no order ID returned. Retcode: " + IntegerToString(result.retcode);
         Print("[TRADE ERROR] ", error);
         SendReport(commandId, false, "", "", error);
      }
   }
