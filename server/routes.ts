...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/webhook", async (req, res) => {
    try {
      ...
      // REMOVED: Heavy signal broadcasting to reduce overhead
      // Signal processing continues without broadcasting

      // Check if auto-trade is enabled
      const settings = await storage.getSettings();
      
      // Check MT5 connection status (aligned with EA's heartbeat timeout)
      const isMt5Connected = settings?.lastMt5Heartbeat && 
        (Date.now() - new Date(settings.lastMt5Heartbeat).getTime() < 30000); // 30 second timeout
  ...
  app.post("/api/settings", async (req, res) => {
    try {
      ...
      // REMOVED: Heavy settings broadcasting to reduce overhead
  ...
  wss.on('connection', (ws: WebSocket) => {
    ...
    // Handle messages from client (e.g., close position requests, keepalive pings)
    ws.on('message', async (data: Buffer) => {
      try {
        const message = JSON.parse(data.toString());
        
        // Don't log keepalive pings to reduce noise
  ...
  mt5Wss.on('connection', async (ws: WebSocket, req) => {
    ...
    // LIGHTWEIGHT: Only update heartbeat on connection, not on every ping
    await storage.updateMt5Heartbeat();

    // LIGHTWEIGHT: Minimal ping only when needed (no frequent database updates)
    const pingInterval = setInterval(async () => {
      if (ws.readyState === WebSocket.OPEN) {
        ws.ping(); // Simple ping, no database writes
      } else {
        clearInterval(pingInterval);
      }
    }, 30000); // Ping every 30 seconds only
    ...
    // REMOVED: Heavy connection stats broadcasting to reduce overhead
    ...
    ws.on('message', async (data: Buffer) => {
      try {
        ...
        // LIGHTWEIGHT: Only log essential trade execution messages
        if (report.commandId) {
          console.log(`[MT5-WS] Trade execution:`, report.commandId, report.success ? 'SUCCESS' : 'FAILED');
        }

        // REMOVED: Heavy account/position broadcasting to reduce overhead
        // Only process trade execution reports

        // Handle execution reports (existing logic)
        const { commandId, success, orderId, positionId, error: errorMessage } = report;

        if (!commandId) {
          console.log('[MT5-WS] Message without commandId (not an execution report), type:', report.type);
          return;
        }
        
        console.log(`[MT5-WS] ✅ EXECUTION REPORT received for command ${commandId}: success=${success}, orderId=${orderId}`);
        ...
        // REMOVED: Heavy stats broadcasting to reduce overhead
        // Only essential trade execution remains
    ...
    ws.on('close', () => {
      ...
      // REMOVED: Heavy disconnect stats broadcasting to reduce overhead
    });
  ...
  // CRITICAL FIX: Fast retry mechanism for pending/sent commands
  // This ensures commands are retried every 5 seconds instead of waiting 30 seconds
  const RETRY_INTERVAL = 5000; // Check every 5 seconds for FAST execution
  const retryPendingCommandsInterval = setInterval(async () => {
    try {
      // First, check for timed-out 'sent' commands and move them back to 'pending'
      // This is CRITICAL - without this, commands stuck in 'sent' status never get retried
      await storage.retryTimedOutCommands();
      
      // Only retry if MT5 is connected
      const stats = await storage.getStats();
      if (!stats.isConnected || mt5WsClients.size === 0) {
        return; // Silent return - no need to log every 5 seconds
      }

      // Get all pending/sent commands (now includes both statuses)
      const pendingCommands = await storage.getPendingCommands();
      
      if (pendingCommands.length === 0) {
        return; // No pending commands, nothing to do
      }
      ...
      // CRITICAL: Only process commands that are currently in 'pending' status
      // Commands in 'sent' status are waiting for acknowledgement and shouldn't be resent
      // until retryTimedOutCommands() converts them back to 'pending' after timeout
      const trulyPendingCommands = pendingCommands.filter(cmd => cmd.status === 'pending');
      
      if (trulyPendingCommands.length === 0) {
        return; // No truly pending commands, nothing to retry
      }
      
      // LIGHTWEIGHT: Only log when actually retrying commands
      if (trulyPendingCommands.length > 0) {
        console.log(`[RETRY] Retrying ${trulyPendingCommands.length} pending commands`);
      }

      for (const command of trulyPendingCommands) {
        // Check command age - if older than 2 minutes, mark as failed
        // Reduced from 5 minutes to 2 minutes for faster failure detection
        const commandAge = Date.now() - new Date(command.createdAt).getTime();
        const TWO_MINUTES = 2 * 60 * 1000;
        
        if (commandAge > TWO_MINUTES) {
          console.log(`[RETRY] Command ${command.id} is too old (${Math.floor(commandAge / 1000)}s), marking as failed`);
          await storage.markCommandAsFailed(command.id, 'Command timeout - exceeded 2 minute limit');
          
          // Update associated signal if exists
          if (command.signalId) {
            await storage.updateSignalStatus(command.signalId, 'failed', 'Trade execution timeout - MT5 connection issue');
            const updatedSignal = await storage.getSignalById(command.signalId);
            if (updatedSignal) {
              broadcast({
                type: 'signal',
        ...
        // LIGHTWEIGHT: Minimal retry logging
        console.log(`[RETRY] Retrying ${command.action} ${command.symbol || 'N/A'}`);
        ...
        if (sent) {
          await storage.markCommandAsSent(command.id);
          // LIGHTWEIGHT: Minimal success logging
          if (command.signalId) {
            await storage.updateSignalStatus(command.signalId, 'pending', 'Retrying execution');
          }
        }
      }
      
      // REMOVED: Heavy retry stats broadcasting to reduce overhead
    } catch (error) {
      console.error('[RETRY] Error in pending command retry:', error);
    }
  }, RETRY_INTERVAL);

  // Store interval for cleanup (optional, for testing)
  (httpServer as any).retryInterval = retryPendingCommandsInterval;

  console.log('[RETRY] Fast pending command retry mechanism initialized (interval: 5s, timeout check enabled)');
