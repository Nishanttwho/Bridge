...
export class MemStorage implements IStorage {
  ...
  async retryTimedOutCommands(): Promise<void> {
    const now = new Date();
    // CRITICAL FIX: Reduced timeout from 30s to 10s for faster retry
    // If a command has been 'sent' for 10s without acknowledgement, move it back to 'pending'
    ...
    for (const command of timedOutCommands) {
      ...
      console.log(`[TIMEOUT] Command ${command.id} timed out after ${timeout/1000}s - returning to pending queue for retry`);
    }
  }
  ...
  async getStats(): Promise<DashboardStats> {
    ...
    // Check if MT5 is connected (heartbeat within last 30 seconds)
    const now = new Date();
    const lastHeartbeat = this.settings?.lastMt5Heartbeat;
    const isConnected = lastHeartbeat 
      ? (now.getTime() - new Date(lastHeartbeat).getTime()) < 30000 
      : false;
